...
// ── Status helpers ─────────────────────────────────────────────────────────
const STATUS = {
    not_configured: { border: 'border-muted-foreground/40', bg: 'bg-muted-foreground/40', text: 'text-muted-foreground', fill: 'bg-muted-foreground/40', light: 'bg-muted' },
    configured: { border: 'border-info', bg: 'bg-info', text: 'text-info', fill: 'bg-info', light: 'bg-info/15' },
    available: { border: 'border-success', bg: 'bg-success', text: 'text-success', fill: 'bg-success', light: 'bg-success/15' },
    booked: { border: 'border-warning', bg: 'bg-warning', text: 'text-warning', fill: 'bg-warning', light: 'bg-warning/15' },
    sold: { border: 'border-admin-accent', bg: 'bg-admin-accent', text: 'text-admin-accent', fill: 'bg-admin-accent', light: 'bg-admin-accent/15' },
};

function statusBorder(s) { return STATUS[s]?.border ?? 'border-muted-foreground/40'; }
function statusBg(s) { return STATUS[s]?.bg ?? 'bg-muted-foreground/40'; }
...
const SECTION_TYPE_COLOR = {
    ...
    office: 'bg-chart-4',
    villa: 'bg-warning',
    industrial: 'bg-muted-foreground',
};
function sectionDot(type) { return SECTION_TYPE_COLOR[type] ?? 'bg-muted-foreground/40'; }
    ...
    <AdminLayout>
        ...
        <div v-if="!buildings.length"
            ...
            <Link :href="route('admin.projects.edit', project.id)"
                class="mt-5 inline-flex items-center gap-2 rounded-xl bg-admin-accent px-5 py-2.5 text-sm font-semibold text-on-gold hover:bg-admin-accent/90 transition-colors">
                Edit Project
            </Link>
        </div>
        ...
        <div v-else class="flex rounded-2xl border border-border bg-admin-surface-card overflow-hidden shadow-sm"
            ...
            <!-- ──────────── LEFT PANEL ──────────────────────────── -->
            <div class="flex flex-col border-r border-border bg-admin-surface-card"
                    ...
                    <div class="flex flex-col gap-1">
                        <button v-for="(s, i) in sections" :key="s.id" @click="activeSection = i" :class="[
                            ...
                            activeSection === i
                                ...
                                : 'border-transparent text-foreground hover:bg-muted font-medium'
                        ]">
                ...
                <div class="flex-1 overflow-y-auto">
                    ...
                    <template v-else>
                        ...
                        <div v-for="floor in floors" :key="floor.floor" @click="activeFloor = floor.floor" :class="[
                            ...
                            <div v-else class="mt-2 h-1.5 w-full rounded-full bg-muted" />
                        </div>
            ...
            <!-- ──────────── CENTRE PANEL ─────────────────────────── -->
            <div class="flex-1 overflow-y-auto bg-admin-surface-page">

                <!-- Block section view -->
                <div v-if="isBlockSection" class="flex h-full min-h-[400px] flex-col items-center justify-center p-8">
                    <div
                        class="w-full max-w-md rounded-2xl border border-amber-200 dark:border-amber-700/40 bg-card shadow-md p-8 text-center">
                        <div
                        ...
                        <template v-if="blockUnit">
                            <div
                                class="mt-5 rounded-xl border border-border bg-muted p-4 text-left">
                            ...
                            <Link :href="route('admin.units.configure', blockUnit.id)"
                                class="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-admin-accent px-5 py-3 text-sm font-semibold text-on-gold hover:bg-admin-accent/90 transition-colors">
                                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                    stroke-width="2">
                ...
                <div v-else class="p-6">
                    ...
                    <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
                        <div class="flex items-center gap-3">
                            <div
                                class="flex h-10 w-10 items-center justify-center rounded-xl bg-admin-accent text-sm font-bold text-on-gold shadow-sm">
                                {{ activeFloor }}
                            </div>
                            <div>
                        ...
                        <div class="flex items-center gap-2">
                            <div
                                class="flex items-center gap-2 rounded-xl border border-border bg-card px-3 py-1.5 shadow-sm">
                            ...
                            <button @click="generateFloor" :disabled="generating || !!currentFloor?.units?.length"
                                ...
                                :class="[
                                    ...
                                    currentFloor?.units?.length
                                        ? 'border border-border bg-card text-muted-foreground cursor-not-allowed opacity-60'
                                        : 'bg-admin-accent text-on-gold hover:bg-admin-accent/90 shadow-sm'
                                ]">
                    ...
                    <div v-if="currentFloor?.units?.length" class="grid gap-2.5"
                        ...
                        <template v-for="unit in currentFloor.units" :key="unit.id">
                            ...
                            <div v-if="isLocked(unit.status)" :class="[
                                'group/tip relative flex flex-col rounded-xl border-2 bg-muted p-3 shadow-sm opacity-70 cursor-not-allowed select-none',
                                ...
                                <span v-if="unit.type_label"
                                    class="mb-1.5 self-start rounded-md bg-muted px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">
                                    {{ unit.type_label }}
                                </span>
                                ...
                                <div class="mt-2 pt-2 border-t border-border/50">
                                    <span class="text-[10px] font-semibold"
                                        :class="STATUS[unit.status]?.text ?? 'text-muted-foreground'">
                                        {{ unit.status_label }}
                                    </span>
                                </div>
                            </div>
                            ...
                            <Link v-else :href="route('admin.units.configure', unit.id)" :class="[
                                'group relative flex flex-col rounded-xl border-2 bg-card p-3 shadow-sm transition-all hover:shadow-md hover:-translate-y-0.5',
                                statusBorder(unit.status)
                            ]">
            ...
            <div class="flex flex-col border-l border-border bg-admin-surface-card"
                        ...
                        <div class="space-y-1.5">
                            <div class="flex items-center justify-between rounded-lg border border-border px-3 py-2">
                                ...
                                <button type="button" @click="qc.servant_room = !qc.servant_room" :class="['relative inline-flex h-5 w-9 flex-none cursor-pointer rounded-full border-2 border-transparent transition-colors',
                                    qc.servant_room ? 'bg-admin-accent' : 'bg-muted']">
                            ...
                            <div class="flex items-center justify-between rounded-lg border border-border px-3 py-2">
                                ...
                                <button type="button" @click="qc.store_room = !qc.store_room" :class="['relative inline-flex h-5 w-9 flex-none cursor-pointer rounded-full border-2 border-transparent transition-colors',
                                    qc.store_room ? 'bg-admin-accent' : 'bg-muted']">
                                    <span :class="['inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform',
                    ...
                    <div
                        ...
                        <div class="flex items-start justify-between gap-2">
                            ...
                            <button type="button" @click="qc.overwrite = !qc.overwrite" :class="['relative mt-0.5 inline-flex h-5 w-9 flex-none cursor-pointer rounded-full border-2 border-transparent transition-colors',
                                qc.overwrite ? 'bg-amber-500' : 'bg-muted']">
                                <span :class="['inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform',
                ...
                <div class="border-t border-border p-4 space-y-2">
                    <button @click="applyQuickConfig" :disabled="applyingConfig || !activeFloor || !qc.type"
                        class="w-full inline-flex h-10 items-center justify-center gap-2 rounded-xl bg-admin-accent text-sm font-semibold text-on-gold hover:bg-admin-accent/90 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm">
