...
<template>
    ...
    <AdminLayout>
        ...
        <div class="mb-5 flex flex-wrap items-start justify-between gap-3">
            ...
            <!-- Legend -->
            <div class="flex flex-wrap items-center gap-2.5 rounded-xl border border-border bg-admin-surface-card px-4 py-2">
        ...
        <div v-if="!buildings.length"
            ...
            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1" class="mb-3 text-muted-foreground/30">
        ...
        <!-- ── 3-panel layout ───────────────────────────────────────── -->
        <div v-else class="flex rounded-2xl border border-border bg-admin-surface-card overflow-hidden shadow-sm" style="min-height: calc(100vh - 195px);">

            <!-- ──────────── LEFT PANEL ──────────────────────────── -->
            <div class="flex flex-col border-r border-border bg-white dark:bg-slate-900/60" style="width:250px; flex-shrink:0;">

                <!-- Building tabs -->
                <div class="border-b border-border p-3">
                    <p class="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Building</p>
                    <div class="flex flex-col gap-1">
                        <button
                            v-for="(b, i) in buildings"
                            :key="b.id"
                            @click="activeBldg = i; activeSection = 0;"
                            :class="[
                        ...
                        >
                            ...
                            <span class="text-[10px] text-muted-foreground">{{ b.total_floors ?? '—' }}F</span>
                        </button>
                    </div>
                </div>
                ...
                <div class="flex-1 overflow-y-auto">
                    ...
                    <div v-if="isBlockSection" class="p-3">
                        <div class="rounded-xl border border-amber-200 dark:border-amber-700/40 bg-amber-50 dark:bg-amber-900/20 p-3 text-center">
                            <svg class="mx-auto mb-1.5 text-amber-500" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                            ...
                            <p class="mt-0.5 text-[10px] text-amber-600/70">Floors {{ currentSection?.floor_start }}–{{ currentSection?.floor_end }}</p>
                    ...
                    <template v-else>
                        <div class="px-3 pt-3 pb-1">
                            <p class="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Floors</p>
                        </div>
                        <div
                        ...
                        >
                            ...
                            <!-- Status strip -->
                            <div v-if="floor.units?.length" class="mt-2 flex h-1.5 w-full gap-px overflow-hidden rounded-full">
                                <template v-for="(count, status) in floorStatusCounts(floor)" :key="status">
                                    <div :class="statusBg(status)"
                                        :style="{ flex: count }"
                                        :title="`${count} ${status}`"
                                    />
                                </template>
            ...
            <div class="flex-1 overflow-y-auto bg-slate-50/60 dark:bg-slate-900/30">
                ...
                <div v-if="isBlockSection" class="flex h-full min-h-[400px] flex-col items-center justify-center p-8">
                    <div class="w-full max-w-md rounded-2xl border border-amber-200 dark:border-amber-700/40 bg-white dark:bg-slate-800 shadow-md p-8 text-center">
                        <div class="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-2xl bg-amber-100 dark:bg-amber-900/30">
                            <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="text-amber-600 dark:text-amber-400">
                                <rect x="3" y="3" width="18" height="18" rx="2"/><rect x="7" y="7" width="10" height="10" rx="1"/>
                            </svg>
                        </div>
                        <span class="inline-flex items-center gap-1.5 rounded-full bg-amber-100 dark:bg-amber-900/30 px-3 py-1 text-xs font-semibold text-amber-700 dark:text-amber-400 mb-3">
                        ...
                        <p class="mt-1 text-sm text-muted-foreground">This entire section is sold as one unit spanning multiple floors.</p>

                        <template v-if="blockUnit">
                            <div class="mt-5 rounded-xl border border-border bg-slate-50 dark:bg-slate-700/50 p-4 text-left">
                                <div class="flex items-start justify-between gap-2">
                                    <div>
                                        <p class="font-semibold text-foreground">{{ blockUnit.unit_number }}</p>
                                        <p class="mt-0.5 text-xs text-muted-foreground">
                                            Floor {{ blockUnit.floor }}–{{ blockUnit.floor_end }}
                                            <template v-if="blockUnit.size_sqft"> · {{ blockUnit.size_sqft.toLocaleString() }} sqft</template>
                                            <template v-if="blockUnit.type_label"> · {{ blockUnit.type_label }}</template>
                                        </p>
                                    </div>
                                    <span :class="['flex-shrink-0 rounded-lg px-2.5 py-1 text-[10px] font-bold text-white', statusBg(blockUnit.status)]">
                        ...
                        <template v-else>
                            <p class="mt-4 text-sm text-muted-foreground">No unit has been generated for this section yet.</p>
                            <button @click="generateBlock" :disabled="generatingBlock"
                                ...
                                <svg v-if="generatingBlock" class="animate-spin" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
                                <svg v-else width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 5v14M5 12h14"/></svg>
                                Generate Block Unit
                            </button>
                        </template>
                    </div>
                ...
                <div v-else-if="!activeFloor" class="flex h-full min-h-[400px] flex-col items-center justify-center gap-4 p-8 text-center">
                    <div class="flex h-16 w-16 items-center justify-center rounded-2xl bg-slate-100 dark:bg-slate-800">
                        <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="text-muted-foreground/50">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-muted-foreground">Pick a floor from the left panel to view and configure its units.</p>
                    </div>
                </div>
                ...
                <div v-else class="p-6">
                    ...
                    <div class="mb-5 flex flex-wrap items-center justify-between gap-3">
                        <div class="flex items-center gap-3">
                            <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-admin-accent text-sm font-bold text-white shadow-sm">
                                {{ activeFloor }}
                            </div>
                            <div>
                                <h2 class="font-semibold text-foreground leading-tight">
                                    Floor {{ activeFloor }}
                                    <span v-if="currentSection" class="text-sm font-normal text-muted-foreground">— {{ currentSection.name }}</span>
                                </h2>
                                <p class="text-xs text-muted-foreground">{{ currentFloor?.units?.length ?? 0 }} units on this floor</p>
                            </div>
                        </div>

                        <!-- Generate controls -->
                        <div class="flex items-center gap-2">
                            <div class="flex items-center gap-2 rounded-xl border border-border bg-white dark:bg-slate-800 px-3 py-1.5 shadow-sm">
                            ...
                            <button @click="generateFloor"
                                :disabled="generating || !!currentFloor?.units?.length"
                            ...
                            >
                                <svg v-if="generating" class="animate-spin" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
                                <svg v-else width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 5v14M5 12h14"/></svg>
                                Generate
                            </button>
                            <button v-if="currentFloor?.units?.length" @click="deleteFloor"
                                class="inline-flex h-9 items-center gap-1.5 rounded-xl border border-red-200 dark:border-red-800/40 bg-red-50 dark:bg-red-900/20 px-3 text-sm font-medium text-red-600 hover:bg-red-100 dark:hover:bg-red-900/40 transition-colors">
                                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M3 6h18M19 6l-1 14H6L5 6M9 6V4h6v2"/></svg>
                                Clear
                            </button>
                        </div>
                    </div>
                    ...
                    <!-- Unit grid -->
                    <div v-if="currentFloor?.units?.length" class="grid gap-2.5" style="grid-template-columns: repeat(auto-fill, minmax(130px, 1fr));">
                        <Link
                        ...
                        >
                            ...
                            <!-- Type badge -->
                            <span v-if="unit.type_label" class="mb-1.5 self-start rounded-md bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground">
                                {{ unit.type_label }}
                            </span>
                            ...
                            <div class="flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[10px] text-muted-foreground">
                                ...
                                <span v-if="unit.bathrooms" class="flex items-center gap-0.5">
                                    <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 6 6.5 3.5a1.5 1.5 0 0 0-1-.5C4.683 3 4 3.683 4 4.5V17a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-5"/><line x1="10" y1="5" x2="8" y2="7"/><line x1="2" y1="12" x2="22" y2="12"/></svg>
                                    {{ unit.bathrooms }}ba
                                </span>
                    ...
                    <!-- Empty floor -->
                    <div v-else class="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border py-16 text-center">
                        <div class="mb-3 flex h-12 w-12 items-center justify-center rounded-xl bg-slate-100 dark:bg-slate-800">
                            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="text-muted-foreground/50">
                                <rect x="3" y="3" width="7" height="9" rx="1"/><rect x="14" y="3" width="7" height="5" rx="1"/>
                                <rect x="14" y="12" width="7" height="9" rx="1"/><rect x="3" y="16" width="7" height="5" rx="1"/>
                            </svg>
                        </div>
                        <p class="font-semibold text-foreground">No units on Floor {{ activeFloor }}</p>
                        <p class="mt-1 text-sm text-muted-foreground">Set a count above and click Generate to create units.</p>
                    </div>
                </div>
            </div>

            <!-- ──────────── RIGHT PANEL: Quick Config ────────────── -->
            <div class="flex flex-col border-l border-border bg-white dark:bg-slate-900/60" style="width:280px; flex-shrink:0;">
                <!-- Panel header -->
                <div class="border-b border-border px-5 py-4">
                    <h3 class="text-sm font-semibold text-foreground">Quick Config</h3>
                    <p class="mt-0.5 text-xs text-muted-foreground">Bulk-apply settings to same-type units on the selected floor</p>
                </div>
                ...
                <div class="flex-1 overflow-y-auto p-4 space-y-4">
                    ...
                    <div class="space-y-1.5">
                        <Label class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Unit Type</Label>
                        <Select v-model="qc.type">
                            <SelectTrigger class="h-9 text-sm">
                                <SelectValue placeholder="Select type" />
                            </SelectTrigger>
                            <SelectContent>
                                <SelectItem v-for="t in enums.unit_types" :key="t.value" :value="t.value">{{ t.label }}</SelectItem>
                            </SelectContent>
                        </Select>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="grid grid-cols-3 gap-2">
                            <div class="space-y-1">
                                ...
                                <Input v-model="qc.bedrooms" type="number" min="0" max="20" placeholder="0" class="h-8 text-xs text-center px-1" />
                            </div>
                            <div class="space-y-1">
                                <p class="text-[10px] text-muted-foreground text-center">Baths</p>
                                <Input v-model="qc.bathrooms" type="number" min="0" max="20" placeholder="0" class="h-8 text-xs text-center px-1" />
                            </div>
                            <div class="space-y-1">
                                <p class="text-[10px] text-muted-foreground text-center">Balc.</p>
                                <Input v-model="qc.balconies" type="number" min="0" max="10" placeholder="0" class="h-8 text-xs text-center px-1" />
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Size / Price -->
                    <div class="space-y-1.5">
                        <Label class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Size & Price</Label>
                        <div class="grid grid-cols-2 gap-2">
                            <div class="space-y-1">
                                <p class="text-[10px] text-muted-foreground">Size (sqft)</p>
                                <Input v-model="qc.size_sqft" type="number" min="1" placeholder="1200" class="h-8 text-xs" />
                            </div>
                            <div class="space-y-1">
                    ...
                    <!-- Facing -->
                    <div class="space-y-1.5">
                        <Label class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Facing</Label>
                    ...
                    <!-- Servant / Store room -->
                    <div class="space-y-2">
                        <Label class="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Extras</Label>
                        <div class="space-y-1.5">
                    ...
                    <!-- Overwrite -->
                    <div class="rounded-xl border border-amber-200 dark:border-amber-700/40 bg-amber-50 dark:bg-amber-900/20 px-3 py-2.5">
                        <div class="flex items-start justify-between gap-2">
                            <div>
                                <p class="text-xs font-semibold text-amber-800 dark:text-amber-300">Overwrite configured</p>
                                <p class="mt-0.5 text-[10px] text-amber-700/70 dark:text-amber-400/70">Also update already-configured units</p>
                            </div>
                            <button type="button" @click="qc.overwrite = !qc.overwrite"
                                :class="['relative mt-0.5 inline-flex h-5 w-9 flex-none cursor-pointer rounded-full border-2 border-transparent transition-colors',
                ...
                <div class="border-t border-border p-4 space-y-2">
                    ...
                    >
                        <svg v-if="applyingConfig" class="animate-spin" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>
                        <svg v-else width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 12l2 2 4-4"/><path d="M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0z"/></svg>
                        Apply to {{ qc.type || '…' }}
                    </button>
        ...
        <Transition
            ...
            <div v-if="toast" :class="[
                ...
                <svg v-else-if="toast.type === 'error'" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M18 6 6 18M6 6l12 12"/></svg>
                <svg v-else width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/></svg>
                {{ toast.message }}
            </div>
        </Transition>
    </AdminLayout>
