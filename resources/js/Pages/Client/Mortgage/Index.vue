    ...
    <ClientLayout title="Mortgage & Financing">
        ...
        <div class="flex items-start justify-between gap-5">
            ...
            <Link :href="route('client.mortgage.explore')"
                ...
                style="background:linear-gradient(100deg,rgb(var(--hv-gold)),rgb(var(--hv-gold-deep))); box-shadow:0 10px 22px -8px rgba(81,50,224,.5); flex:none;">
        ...
        <div v-if="isDummy" class="flex items-center gap-3 p-4 rounded-2xl border"
            ...
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="hsl(var(--warning))" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="flex-none"><circle cx="12" cy="12" r="9"/><path d="M12 8v4M12 16h.01"/></svg>
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    <div v-for="card in cards" :key="card.id ?? card.unit_number"
                        class="relative rounded-2xl border bg-client-surface-card border-[hsl(var(--border))] dark:border-white/[0.06] shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-shadow">

                        <!-- Cover image -->
                        <div class="relative h-[160px] bg-gradient-to-br flex items-center justify-center flex-none"
                            style="background:linear-gradient(135deg,rgba(198,161,91,0.1),rgba(198,161,91,0.2));">
                            <img v-if="card.cover_image" :src="card.cover_image" :alt="card.project_name"
                                class="absolute inset-0 w-full h-full object-cover" />
                            <svg v-else width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="rgb(var(--hv-gold-deep))" stroke-width="1.3" stroke-linecap="round" stroke-linejoin="round" class="opacity-70">
                        ...
                        <div class="p-4 flex-1 flex flex-col">
                            ...
                            <div class="flex flex-wrap items-center gap-2 mt-3">
                                <span class="text-xs font-bold px-2 py-0.5 rounded-md bg-[rgba(198,161,91,0.1)] text-client-accent">{{ card.type_label }}</span>
                            ...
                            <!-- Price -->
                            <div class="mt-4 pt-3.5 border-t border-[hsl(var(--muted))] dark:border-white/[0.06]">
                            ...
                            <div class="mt-auto pt-4">
                                <Link :href="calcHref(card)"
                                    ...
                                    style="background:linear-gradient(100deg,rgb(var(--hv-gold)),rgb(var(--hv-gold-deep))); box-shadow:0 8px 18px -6px rgba(81,50,224,.45);">
            ...
            <aside class="hidden xl:flex flex-col gap-4" style="width:296px; flex:none;">
                ...
                <div class="rounded-2xl border bg-client-surface-card border-[hsl(var(--border))] dark:border-white/[0.06] p-5 shadow-sm">
                    <div class="flex items-center gap-2.5 mb-3.5">
                        <span class="w-9 h-9 flex-none rounded-xl bg-[rgba(198,161,91,0.1)] flex items-center justify-center text-client-accent">
                    ...
                    <div class="mt-3.5 flex flex-col gap-2.5">
                        <div class="flex items-start gap-2 text-xs text-muted-foreground font-medium">
                            <span class="w-4 h-4 flex-none rounded-full bg-[rgba(198,161,91,0.1)] flex items-center justify-center text-[8.5px] font-extrabold text-client-accent mt-0.5">1</span>
                            Choose a property and agree on the price.
                        </div>
                        <div class="flex items-start gap-2 text-xs text-muted-foreground font-medium">
                            <span class="w-4 h-4 flex-none rounded-full bg-[rgba(198,161,91,0.1)] flex items-center justify-center text-[8.5px] font-extrabold text-client-accent mt-0.5">2</span>
                            Pay a down payment (typically 20–40%).
                        </div>
                        <div class="flex items-start gap-2 text-xs text-muted-foreground font-medium">
                            <span class="w-4 h-4 flex-none rounded-full bg-[rgba(198,161,91,0.1)] flex items-center justify-center text-[8.5px] font-extrabold text-client-accent mt-0.5">3</span>
                ...
                <div class="rounded-2xl border bg-client-surface-card border-[hsl(var(--border))] dark:border-white/[0.06] p-5 shadow-sm">
                    ...
                    <div class="grid grid-cols-2 gap-2.5">
                        <div class="flex items-center justify-center h-12 border border-[hsl(var(--muted))] dark:border-white/[0.06] rounded-xl text-xs font-extrabold" style="color:#1b3a6b;">BRAC Bank</div>
                        <div class="flex items-center justify-center h-12 border border-[hsl(var(--muted))] dark:border-white/[0.06] rounded-xl text-xs font-extrabold" style="color:#c0392b;">City Bank</div>
                        <div class="flex items-center justify-center h-12 border border-[hsl(var(--muted))] dark:border-white/[0.06] rounded-xl text-xs font-extrabold" style="color:#1d7a46;">Islami Bank</div>
                        <div class="flex items-center justify-center h-12 border border-[hsl(var(--muted))] dark:border-white/[0.06] rounded-xl text-xs font-extrabold" style="color:#2c3e8c;">Prime Bank</div>
                    </div>
                </div>

                <!-- Get Pre-Approved promo -->
                <div class="relative overflow-hidden rounded-2xl p-5 border border-[rgba(198,161,91,0.08)]"
                    style="background:linear-gradient(135deg,rgba(198,161,91,0.1),rgba(198,161,91,0.08));">
                    <div style="max-width:170px;">
                        <div class="text-sm font-bold text-[rgb(var(--hv-gold-deep))]">Get Pre-Approved</div>
                        <div class="text-xs font-medium leading-relaxed text-[rgb(var(--brand-text))] mt-1.5 mb-4">Connect with our banking partners for the best rates on your property.</div>
                        <button class="text-xs font-bold px-4 py-2.5 rounded-xl text-white border-0 cursor-pointer"
                            style="background:linear-gradient(100deg,rgb(var(--hv-gold)),rgb(var(--hv-gold-deep))); box-shadow:0 6px 14px -4px rgba(81,50,224,.55);">
                            Apply Now
                        </button>
                    </div>
                    <div class="absolute right-[-6px] bottom-[-6px] w-[76px] h-[76px] rounded-2xl flex items-center justify-center"
                        style="background:linear-gradient(160deg,rgb(var(--hv-gold)),rgb(var(--hv-gold-deep))); box-shadow:0 12px 24px -8px rgba(79,51,214,.6);">
                        <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="rgb(var(--hv-gold))" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 11 12 4l9 7"/><path d="M5 10v10h14V10"/><path d="M9 20v-5h6v5"/></svg>
                    </div>
                </div>
            </aside>
            <!-- ===== END right rail ================================= -->
        ...
        <!-- ── Footer ─────────────────────────────────────────────── -->
        <footer class="flex flex-wrap items-center justify-between gap-4 rounded-2xl border bg-client-surface-card border-[hsl(var(--border))] dark:border-white/[0.06] px-5 py-3.5 shadow-sm">
