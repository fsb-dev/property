...
const statusMap = {
    ...
    handed_over: { label: 'Handed Over', bg: 'bg-[rgba(198,161,91,0.1)] text-violet-700 dark:bg-violet-500/15 dark:text-violet-400' },
};
...
<template>
    ...
    <ClientLayout title="My Properties">
        ...
        <div class="flex items-start justify-between gap-5 flex-wrap">
            ...
            <!-- Search -->
            <div class="flex items-center gap-2.5 h-11 px-3.5 bg-client-surface-card border border-[hsl(var(--border))] dark:border-white/[0.06] rounded-xl min-w-[260px]">
                <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="hsl(var(--muted-foreground))" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.2-3.2"/></svg>
                <input
                    v-model="searchQuery"
        ...
        <!-- ── Tabs ─────────────────────────────────────────────────── -->
        <div class="mt-5 flex items-center gap-1 border-b border-[hsl(var(--border))] dark:border-white/[0.06]">
            <button
            ...
            >
                ...
                <span v-if="tab.count > 0"
                    ...
                    :class="activeTab === tab.key
                        ...
                        : 'bg-[hsl(var(--muted))] dark:bg-white/[0.06] text-muted-foreground'"
                >{{ tab.count }}</span>
            </button>
        </div>
            ...
            <div class="flex-1 min-w-0 flex flex-col gap-4">
                ...
                <template v-if="activeTab === 'all'">
                    ...
                    <template v-if="purchasedProperties.length">
                        ...
                        <Link v-for="p in purchasedProperties" :key="p.id"
                            ...
                            class="rounded-2xl border bg-client-surface-card border-[hsl(var(--border))] dark:border-white/[0.06] overflow-hidden shadow-sm flex hover:shadow-md hover:border-client-accent/30 transition-all"
                        >
                            <!-- Cover image -->
                            <div class="w-[260px] flex-none relative hidden sm:block" style="min-height:220px;">
                                ...
                                <div v-else class="absolute inset-0 flex items-center justify-center" style="background:linear-gradient(135deg,rgba(198,161,91,0.08),rgba(198,161,91,0.15));">
                                    <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="rgb(var(--hv-gold-deep))" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 10.5 12 3l9 7.5"/><path d="M5 9.5V21h14V9.5"/><path d="M10 21v-6h4v6"/></svg>
                                </div>
                                <!-- Status badge -->
                            ...
                            <div class="flex-1 min-w-0 p-5 flex flex-col">
                                ...
                                <!-- 4-col stats -->
                                <div class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3 py-4 border-t border-b border-[hsl(var(--muted))] dark:border-white/[0.06]">
                                ...
                                <div class="mt-4">
                                    ...
                                    <div class="h-2 rounded-full bg-[rgba(198,161,91,0.08)] dark:bg-client-accent/10 overflow-hidden">
                                        <div class="h-full rounded-full" style="background:linear-gradient(90deg,rgb(var(--hv-gold)),rgb(var(--hv-gold-deep)));" :style="`width:${Math.round(p.construction_pct)}%`"></div>
                                    </div>
                                </div>

                                <span class="mt-4 self-start inline-flex items-center gap-2 text-sm font-bold px-4 py-2.5 rounded-xl text-white"
                                    style="background:linear-gradient(100deg,rgb(var(--hv-gold)),rgb(var(--hv-gold-deep))); box-shadow:0 10px 22px -8px rgba(81,50,224,.5);">
                    ...
                    <template v-if="reservedProperties.length">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <Link v-for="p in reservedProperties" :key="p.id"
                                :href="p.project_id ? route('client.projects.show', p.project_id) : '#'"
                                class="block rounded-2xl border bg-client-surface-card border-[hsl(var(--border))] dark:border-white/[0.06] p-4 shadow-sm hover:shadow-md hover:border-client-accent/30 transition-all"
                            >
                                <div class="flex gap-4">
                                    ...
                                    <div class="w-[100px] h-[100px] flex-none relative rounded-xl overflow-hidden">
                                        <img v-if="p.cover_image" :src="p.cover_image" :alt="p.project_name"
                                            class="absolute inset-0 w-full h-full object-cover" />
                                        <div v-else class="absolute inset-0 flex items-center justify-center bg-[rgba(198,161,91,0.08)] dark:bg-client-accent/10">
                                            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="rgb(var(--hv-gold-deep))" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 10.5 12 3l9 7.5"/><path d="M5 9.5V21h14V9.5"/></svg>
                                        </div>
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="flex items-center gap-2 mt-1 text-xs text-muted-foreground font-medium flex-wrap">
                                            ...
                                            <span v-if="p.block && p.floor" class="w-1 h-1 rounded-full bg-muted-foreground/40"></span>
                                            <span v-if="p.floor">Floor {{ p.floor }}</span>
                                            <span v-if="p.bedrooms" class="w-1 h-1 rounded-full bg-muted-foreground/40"></span>
                                ...
                                <div class="flex gap-2 mt-3">
                                    <button class="flex-1 text-xs font-bold py-2.5 rounded-xl border transition-colors"
                                        style="border-color:rgba(198,161,91,0.1); background:rgba(198,161,91,0.08); color:rgb(var(--hv-gold-deep));"
                                    ...
                                    <span class="px-4 text-xs font-bold py-2.5 rounded-xl border border-[hsl(var(--border))] dark:border-white/[0.06] bg-client-surface-card text-muted-foreground text-center">
                                        View Details
                                    </span>
                                </div>
                            </Link>
                    ...
                    <div v-if="favouriteUnits.length" class="grid grid-cols-2 md:grid-cols-3 gap-4">
                        <Link v-for="u in favouriteUnits" :key="u.id"
                            ...
                            class="block rounded-2xl border bg-client-surface-card border-[hsl(var(--border))] dark:border-white/[0.06] overflow-hidden shadow-sm hover:shadow-md hover:border-client-accent/30 transition-all"
                        >
                            <!-- Image area -->
                            <div class="relative h-[120px] overflow-hidden">
                                <img v-if="u.cover_image" :src="u.cover_image" :alt="u.project_name"
                                    class="absolute inset-0 w-full h-full object-cover" />
                                <div v-else class="absolute inset-0 flex items-center justify-center" style="background:linear-gradient(135deg,rgba(198,161,91,0.08),rgba(198,161,91,0.15));">
                                    <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="rgb(var(--hv-gold-deep))" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 10.5 12 3l9 7.5"/><path d="M5 9.5V21h14V9.5"/></svg>
                                </div>
                                ...
                                <button class="absolute top-2.5 right-2.5 w-8 h-8 rounded-full bg-white shadow-md flex items-center justify-center hover:scale-110 transition-transform" @click.stop.prevent>
                                    <svg width="16" height="16" viewBox="0 0 24 24" fill="#F472B6"><path d="M12 21s-7-4.6-9.3-9C1.2 9 2.6 5.5 6 5.5c2 0 3.2 1.2 4 2.4.8-1.2 2-2.4 4-2.4 3.4 0 4.8 3.5 3.3 6.5C19 16.4 12 21 12 21Z"/></svg>
                                </button>
                            </div>
                            <!-- Info -->
                            <div class="p-3">
                                ...
                                <div class="flex items-center gap-1.5 mt-1 text-xs text-muted-foreground font-medium flex-wrap">
                                    ...
                                    <span v-if="u.floor" class="flex items-center gap-1"><span v-if="u.block" class="w-1 h-1 rounded-full bg-muted-foreground/40"></span>Floor {{ u.floor }}</span>
                                    <span v-if="u.bedrooms" class="flex items-center gap-1"><span class="w-1 h-1 rounded-full bg-muted-foreground/40"></span>{{ u.bedrooms }} Bed</span>
                                    <span v-else-if="u.type_label" class="flex items-center gap-1"><span class="w-1 h-1 rounded-full bg-muted-foreground/40"></span>{{ u.type_label }}</span>
                                </div>
                                <div class="flex items-center justify-between mt-3">
                                    <span class="text-sm font-extrabold text-client-accent">{{ fmtBDT(u.price) }}</span>
                                    <span class="w-8 h-8 rounded-lg border border-[hsl(var(--border))] dark:border-white/[0.06] bg-client-surface-card flex items-center justify-center">
                ...
                <template v-else-if="activeTab !== 'favourites'">
                    <div v-if="tableProperties.length"
                        class="rounded-2xl border bg-client-surface-card border-[hsl(var(--border))] dark:border-white/[0.06] shadow-sm overflow-hidden"
                    >
                        <!-- Table header -->
                        <div class="grid items-center gap-3 px-5 py-3 border-b border-[hsl(var(--muted))] dark:border-white/[0.06] bg-[hsl(var(--background))] dark:bg-white/[0.02]"
                        ...
                        <Link v-for="(p, i) in tableProperties" :key="p.id"
                            :href="p.project_id ? route('client.projects.show', p.project_id) : '#'"
                            class="grid items-center gap-3 px-5 py-3.5 transition-colors hover:bg-[hsl(var(--background))] dark:hover:bg-white/[0.02]"
                            :class="i < tableProperties.length - 1 ? 'border-b border-[hsl(var(--border))] dark:border-white/[0.04]' : ''"
                            style="grid-template-columns:2.2fr 1.2fr 1fr 1.3fr 1.2fr auto;"
                        >
                            <!-- Property -->
                            <div class="flex items-center gap-3 min-w-0">
                                <div class="w-14 h-14 flex-none relative rounded-xl overflow-hidden">
                                    <img v-if="p.cover_image" :src="p.cover_image" :alt="p.project_name"
                                        class="absolute inset-0 w-full h-full object-cover" />
                                    <div v-else class="absolute inset-0 flex items-center justify-center bg-[rgba(198,161,91,0.08)] dark:bg-client-accent/10">
                                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgb(var(--hv-gold-deep))" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 10.5 12 3l9 7.5"/><path d="M5 9.5V21h14V9.5"/></svg>
                                    </div>
                                </div>
                                <div class="min-w-0">
                            ...
                            <div class="flex flex-col gap-1">
                                ...
                                <div class="h-1.5 rounded-full bg-[rgba(198,161,91,0.08)] dark:bg-client-accent/10 overflow-hidden mt-0.5" style="width:80px;">
                            ...
                            <div class="flex flex-col gap-1.5">
                                ...
                                <div class="h-1.5 rounded-full bg-[rgba(198,161,91,0.08)] dark:bg-client-accent/10 overflow-hidden" style="width:110px;">
                                    <div class="h-full rounded-full" style="background:linear-gradient(90deg,rgb(var(--hv-gold)),rgb(var(--hv-gold-deep)));"
                        ...
                        >
                            ...
                            <div class="p-3.5">
                                ...
                                <div class="flex items-center gap-2 mt-1 text-xs text-muted-foreground font-medium">
                                    ...
                                    <span v-if="u.bedrooms && u.size_sqft" class="w-1 h-1 rounded-full bg-muted-foreground/40"></span>
                                    <span v-if="u.size_sqft">{{ Number(u.size_sqft).toLocaleString() }} sft</span>
                                    <span v-else>{{ u.type_label }}</span>
                                </div>
                                <div class="flex items-center justify-between mt-3">
                                    <span class="text-sm font-extrabold text-client-accent">{{ fmtBDT(u.price) }}</span>
                                    <span class="w-8 h-8 rounded-lg border border-[hsl(var(--border))] dark:border-white/[0.06] flex items-center justify-center">
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="text-client-accent"><path d="M12 4v16M5 8 3 12h4zM19 8l-2 4h4z"/><path d="M3 12c.7 1.3 2 2 4 2s3.3-.7 4-2M13 12c.7 1.3 2 2 4 2s3.3-.7 4-2"/></svg>
                                    </span>
                                </div>
                            </div>
            ...
            <aside class="hidden xl:flex flex-col gap-4" style="width:300px; flex:none;">
                ...
                <div class="rounded-2xl border bg-client-surface-card border-[hsl(var(--border))] dark:border-white/[0.06] p-5 shadow-sm">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div class="rounded-xl border border-[hsl(var(--muted))] dark:border-white/[0.06] p-3.5">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="hsl(var(--success))" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 10.5 12 3l9 7.5"/><path d="M5 9.5V21h14V9.5"/><path d="M10 21v-6h4v6"/></svg>
                        ...
                        <div class="rounded-xl border border-[hsl(var(--muted))] dark:border-white/[0.06] p-3.5">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="hsl(var(--warning))" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 3h12v18l-6-4-6 4z"/></svg>
                        ...
                        <div class="rounded-xl border border-[hsl(var(--muted))] dark:border-white/[0.06] p-3.5">
                            <svg width="20" height="20" viewBox="0 0 24 24" fill="#F472B6"><path d="M12 21s-7-4.6-9.3-9C1.2 9 2.6 5.5 6 5.5c2 0 3.2 1.2 4 2.4.8-1.2 2-2.4 4-2.4 3.4 0 4.8 3.5 3.3 6.5C19 16.4 12 21 12 21Z"/></svg>
                ...
                <div class="rounded-2xl border bg-client-surface-card border-[hsl(var(--border))] dark:border-white/[0.06] p-5 shadow-sm">
                    ...
                    <div class="flex gap-3 items-center">
                        <div class="w-[68px] h-[68px] flex-none rounded-xl overflow-hidden relative">
                            <img v-if="handoverData.cover_image" :src="handoverData.cover_image" :alt="handoverData.project_name"
                                class="absolute inset-0 w-full h-full object-cover" />
                            <div v-else class="absolute inset-0 flex items-center justify-center bg-[rgba(198,161,91,0.08)] dark:bg-client-accent/10">
                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="rgb(var(--hv-gold-deep))" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 10.5 12 3l9 7.5"/><path d="M5 9.5V21h14V9.5"/></svg>
                            </div>
                        </div>
                        <div class="min-w-0">
                    ...
                    <div class="mt-4 flex items-center justify-between gap-3 p-3.5 bg-[rgba(198,161,91,0.08)] dark:bg-client-accent/10 rounded-xl">
                        <div>
                    ...
                    <button class="mt-3 w-full text-sm font-bold py-2.5 rounded-xl border transition-colors"
                        style="border-color:rgba(198,161,91,0.1); color:rgb(var(--hv-gold-deep)); background:hsl(var(--card));"
                        @mouseenter="$event.currentTarget.style.background='rgba(198,161,91,0.08)'"
                        @mouseleave="$event.currentTarget.style.background='hsl(var(--card))'"
                ...
                <div class="rounded-2xl border bg-client-surface-card border-[hsl(var(--border))] dark:border-white/[0.06] p-5 shadow-sm">
                    ...
                    <div class="flex flex-col gap-1">
                        <a v-for="action in [
                            { label:'Compare Properties',  well:'bg-[rgba(198,161,91,0.1)]',                      ic:'text-client-accent', icon:`<path d='M12 4v16M5 8 3 12h4zM19 8l-2 4h4z'/><path d='M3 12c.7 1.3 2 2 4 2s3.3-.7 4-2M13 12c.7 1.3 2 2 4 2s3.3-.7 4-2'/>` },
                            ...
                            href="#"
                            class="flex items-center gap-3 p-2.5 rounded-xl transition-colors hover:bg-[hsl(var(--background))] dark:hover:bg-white/[0.04]"
                        >
