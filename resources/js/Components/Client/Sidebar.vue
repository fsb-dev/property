    ...
    >
        ...
        <div style="display:flex; align-items:center; gap:11px; padding:4px 8px 22px; flex-shrink:0;">
            ...
            <div style="display:flex; align-items:flex-end; gap:3px; height:34px; flex:none;">
                ...
                <div style="width:7px; height:34px; border-radius:3px; background:linear-gradient(180deg,#7b63ff,#4f33d6);"></div>
                <div style="width:7px; height:26px; border-radius:3px; background:#241a5c;"></div>
            </div>
            <div style="line-height:1; flex:1;">
                <div class="text-foreground" style="font-size:19px; font-weight:800; letter-spacing:-0.02em;">LakeView</div>
                <div style="font-size:9.5px; font-weight:600; letter-spacing:0.32em; color:#9a9aac; margin-top:3px;">RESIDENCES</div>
            </div>
            ...
            <button
                ...
                @click="emit('close')"
            >
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" class="text-muted-foreground">
        ...
        <nav style="display:flex; flex-direction:column; gap:3px; flex:1;">
            <template v-for="link in navLinks" :key="link.label">
                <Link
                    ...
                    @click="emit('close')"
                >
                    <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="flex:none;" v-html="link.icon" />
                    {{ link.label }}
                </Link>
        ...
        <div style="margin-top:auto; display:flex; flex-direction:column; gap:12px; padding-top:14px; flex-shrink:0;">
            <!-- AI help promo card -->
            <div style="position:relative; overflow:hidden; border-radius:18px; padding:16px; background:linear-gradient(135deg,#efeafc,#e7e0ff); border:1px solid #e3daff;">
                <div style="max-width:128px;">
                    <div style="display:inline-flex; align-items:center; gap:6px; font-size:13px; font-weight:700; color:#3a25b0;">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="#5b3fe8"><path d="M12 2l1.8 5.6L19.5 9l-4.6 3.4L16.5 18 12 14.7 7.5 18l1.6-5.6L4.5 9l5.7-1.4z"/></svg>
                        Need Help?
                    </div>
                    <div style="font-size:12px; line-height:1.5; color:#6a5fae; margin:7px 0 12px; font-weight:500;">Chat with your AI Property Advisor.</div>
                    <button style="border:none; cursor:pointer; background:linear-gradient(100deg,#6a4dff,#5132e0); color:#fff; font-size:12.5px; font-weight:700; padding:9px 18px; border-radius:10px; box-shadow:0 6px 14px -4px rgba(81,50,224,.55); font-family:inherit;">
                        Chat Now
                    </button>
                </div>
                <!-- Robot icon -->
                <div style="position:absolute; right:-6px; bottom:-6px; width:88px; height:88px; border-radius:24px; background:linear-gradient(160deg,#7b63ff,#4f33d6); display:flex; align-items:center; justify-content:center; box-shadow:0 12px 24px -8px rgba(79,51,214,.6);">
                    <div style="width:46px; height:38px; background:#1a123f; border-radius:13px; position:relative;">
                        <div style="position:absolute; top:11px; left:9px; width:9px; height:9px; border-radius:50%; background:#8fb4ff;"></div>
                        <div style="position:absolute; top:11px; right:9px; width:9px; height:9px; border-radius:50%; background:#8fb4ff;"></div>
                        <div style="position:absolute; top:-7px; left:50%; transform:translateX(-50%); width:2px; height:7px; background:#cdbcff;"></div>
                        <div style="position:absolute; top:-9px; left:50%; transform:translateX(-50%); width:5px; height:5px; border-radius:50%; background:#cdbcff;"></div>
                    </div>
                </div>
            </div>
            ...
            <div ref="profileRef" class="relative">
                ...
                >
                    ...
                    >
                        ...
                        <button class="flex w-full items-center gap-2.5 px-3 py-2.5 text-sm text-foreground transition-colors hover:bg-muted">
                ...
                <button
                    ...
                    style="gap:11px; padding:11px 12px;"
                    @click="profileOpen = !profileOpen"
                >
                    <div
                        class="flex-none flex items-center justify-center rounded-full text-white"
                        style="width:40px; height:40px; background:linear-gradient(135deg,#c9bdf7,#8b6df0); font-size:15px; font-weight:700;"
                    >
                        {{ initials(client?.name) }}
                    </div>
                    <div class="min-w-0 flex-1">
                        <div class="text-foreground font-bold truncate" style="font-size:13.5px;">{{ client?.name ?? 'Buyer' }}</div>
                        <div class="text-muted-foreground truncate" style="font-size:11.5px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">{{ client?.email ?? '' }}</div>
                    </div>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#b6b6c4" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" class="flex-none">
                        <path d="M9 6l6 6-6 6"/>
                    </svg>
                </button>
            </div>
